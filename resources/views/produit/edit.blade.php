...
<div class="container-fluid">
    ...
    <div class="row">
      ...
      <div class="col-md-12 col-lg-12">
        <div class="card">
		  ...
		  <div class="card-body">
			<form action="{{ route('cadre_logiques.produits.update', [$cadre_logique->id, $produit->id]) }}" method="POST">
				@csrf
				@method('PUT')
				<div class="row">
					<div class="row mb-3">
						<div class="col-md-4">
						  ...
						  <input name="intitule" type="text" class="form-control" value="{{ old('intitule', $produit->intitule) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Type produit</label>
						   <select id="type_produit_id" name="type_produit_id" class="form-select @error('typeProduit') is-invalid @enderror">
						...
						<div class="col-md-4">
						  ...
						  <input name="cout_prevu" type="number" class="form-control" value="{{ old('cout_prevu', $produit->cout_prevu) }}">
						</div>
					</div>
					<div class="row mb-3">
						<div class="col-md-4">
						  <label class="form-label">Année début (prévu)</label>
						  <input name="annee_debut_prevu" type="number" class="form-control" value="{{ old('annee_debut_prevu', $produit->annee_debut_prevu) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Année fin (prévu)</label>
						  <input name="annee_fin_prevu" type="number" class="form-control" value="{{ old('annee_fin_prevu', $produit->annee_fin_prevu) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Durée des travaux (mois)</label>
						  <input name="duree_travaux" type="number" class="form-control" value="{{ old('duree_travaux', $produit->duree_travaux) }}">
						</div>
					</div>
					<div class="row mb-3">
						<div class="col-md-4">
						  <label class="form-label">Responsable</label>
						  <input name="responsable" type="text" class="form-control" value="{{ old('responsable', $produit->responsable) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Contact Responsable</label>
						  <input name="contact_responsable" type="text" class="form-control" value="{{ old('contact_responsable', $produit->contact_responsable) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Statut produit</label>
						   <select id="statut_produit_id" name="statut_produit_id" class="form-select @error('statutProduit') is-invalid @enderror">
								<option value="">-- Sélectionner un statut --</option>
								@foreach($statutProduits as $statut)
									<option value="{{ $statut->id }}"
										{{ old('statut_produit_id', $produit->statut_produit_id ?? '') == $statut->id ? 'selected' : '' }}>
										{{ $statut->intitule }}
									</option>
								@endforeach
							</select>
				...
				<div class="row infos_realisation">
					<div class="row mb-3">
						<div class="col-md-4">
						  ...
						  <input name="date_debut_realisation" type="date" class="form-control" value="{{ old('date_debut_realisation', $produit->date_debut_realisation) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Date fin réalisation</label>
						  <input name="date_fin_realisation" type="date" class="form-control" value="{{ old('date_fin_realisation', $produit->date_fin_realisation) }}">
						</div>
						<div class="col-md-4">
						  <label class="form-label">Coût de réalisation (FCFA)</label>
						  <input name="cout_realisation" type="number" class="form-control" value="{{ old('cout_realisation', $produit->cout_realisation) }}">
						</div>
					</div>
				</div>
				<div class="row infos_realisation">
					<div class="row mb-3">
						<div class="col-md-6">
						  ...
						  <input name="latitude" type="text" class="form-control" value="{{ old('Latitude', $produit->Latitude) }}">
						</div>
						<div class="col-md-6">
						  <label class="form-label">Longitude</label>
						  <input name="longitude" type="text" class="form-control" value="{{ old('longitude', $produit->longitude) }}">
						</div>
					</div>
				</div>
	...
	<script>
		...
		$(document).ready(function () {
			...
			$('#statut_produit_id').change(function(){
				if($(this).val() != 3)
				{
