...
class DashboardController extends Controller
{
     ...
     *
     * NB: un projet peut être rattaché à une zone à n'importe quel niveau
     * de la hiérarchie (1=pays, 2=région, 3=département, 4=commune) via
     * projet_zone. Un simple JOIN filtré sur z.niveau = 2 ne récupère donc
     * que les projets rattachés EXPLICITEMENT à une région, et ignore tous
     * ceux rattachés à un département/commune (cas le plus fréquent), d'où
     * un graphique vide ou incomplet.
     *
     * On remonte donc la hiérarchie des zones via une requête récursive
     * (WITH RECURSIVE) pour retrouver la région ancêtre de la zone associée
     * au projet, quel que soit le niveau de rattachement.
     */
    private function getFinancementParRegion()
    {
        $sql = "
            WITH RECURSIVE zone_ancestors AS (
                -- Point de départ : les zones de niveau région elles-mêmes
                SELECT id AS zone_id, id AS region_id, intitule AS region
                FROM zones
                WHERE niveau = 2

                UNION ALL

                -- On descend récursivement vers les enfants (département, commune, ...)
                -- en propageant l'identifiant/nom de la région ancêtre
                SELECT z.id AS zone_id, za.region_id, za.region
                FROM zones z
                INNER JOIN zone_ancestors za ON z.zone_id = za.zone_id
            )
            SELECT za.region, SUM(ppf.montant) AS montant
            FROM projet_plan_financements ppf
            INNER JOIN projets p ON ppf.projet_id = p.id
            INNER JOIN projet_zone pz ON p.id = pz.projet_id
            INNER JOIN zone_ancestors za ON pz.zone_id = za.zone_id
            WHERE ppf.deleted_on IS NULL
              AND p.deleted_on IS NULL
              
            GROUP BY za.region_id, za.region
    ...
    /**
     * Financement par bailleur (Top 10)
     *
     * - LEFT JOIN plutôt que INNER JOIN sur bailleurs : une ligne de
     *   financement dont le bailleur_id ne correspondrait à aucun bailleur
     *   existant (donnée historique/importée) n'est plus silencieusement
     *   exclue du graphique, elle est regroupée sous "Non spécifié".
     * - Ajout du filtre sur les projets supprimés (whereNull p.deleted_on),
     *   absent de la requête d'origine, pour ne pas comptabiliser les
     *   financements de projets supprimés.
     */
    private function getFinancementParBailleur()
    {
        return DB::table('projet_plan_financements as ppf')
            ->join('projets as p', 'ppf.projet_id', '=', 'p.id')
            ->leftJoin('bailleurs as b', 'ppf.bailleur_id', '=', 'b.id')
            ->select(
                DB::raw("COALESCE(b.intitule, 'Non spécifié') as bailleur"),
                DB::raw('SUM(ppf.montant) as montant')
            )
            ->whereNull('ppf.deleted_on')
            ->whereNull('p.deleted_on')
            ->whereNotNull('ppf.montant')
